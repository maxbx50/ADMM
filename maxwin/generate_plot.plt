...
plot 'err_curl.csv' every ::1::80 w lp t '$\|y_{k,h}-\bar y_h\|_{L^2(\Omega)}$ for $Y=H(\mbox{curl};\Omega)$' ls 1, \
     'dist_curl.csv' every ::1::80 w lp t '$\|y_{k,h}-y_{k+1,h}\|_{L^2(\Omega)}$ for for $Y=H(\mbox{curl};\Omega)$' ls 2, \
     'eps_curl.csv' every ::1::80 w l t '$\varepsilon_k$' ls 3, \
     'err_l2.csv' every ::1::80 w lp t '$\|y_{k,h}-\bar y_h\|_{L^2(\Omega)}$ for $Y=L^2(\Omega)$' ls 4, \
     'dist_l2.csv' every ::1::80 w lp t '$\|y_{k,h}-y_{k+1,h}\|_{L^2(\Omega)}$ for $Y=L^2(\Omega)$' ls 5